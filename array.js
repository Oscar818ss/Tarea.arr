...
function procesarPedido(pedido) {
    ...
        return pedido; // Retorna el nuevo array
    }
    ...
        return index !== -1 ? words.slice(index + 1) : []; // Devuelve las palabras después del índice o un array vacío si no la encuentra
    }
    ...
            if (index !== -1) {
                return [i, index]; // Retorna la posición si lo encuentra
            }
        }
        return [-1, -1]; // Retorna [-1, -1] si no lo encuentra
    }
    ...
            }
        }
        return [minIndex, maxIndex]; // Retorna ambos índices
    }
